jQuery('document').ready(function(){
	jQuery('button').on('click', function(){
		...
		if(jQuery('#bal5').val() == "" || jQuery('#bal4').val() == "" || jQuery('#bal3').val() == ""
			|| jQuery('#bal2').val() == "" || jQuery('#bal1').val() == "" || jQuery('#bal5').val() > 200 || jQuery('#bal4').val() > 200 || jQuery('#bal3').val() > 200
			|| jQuery('#bal2').val() > 200 || jQuery('#bal').val() > 200 || jQuery('#bal5').val() < 100 || jQuery('#bal4').val() < 100 || jQuery('#bal3').val() < 100
			|| jQuery('#bal2').val() < 100 || jQuery('#bal').val() < 100){
		if(jQuery('#bal5').val() == "" || jQuery('#bal4').val() == "" || jQuery('#bal3').val() == ""
			|| jQuery('#bal2').val() == "" || jQuery('#bal1').val() == ""){
			alert("Введіть дані, якщо у вас немає цих балів, введіть 0.");
		}
		if(jQuery('#bal5').val() > 200 || jQuery('#bal4').val() > 200 || jQuery('#bal3').val() > 200
			|| jQuery('#bal2').val() > 200 || jQuery('#bal').val() > 200){
			alert("Бали ЗНО, ФДП та атестату не можуть перевищувати 200.");
		}
		if(jQuery('#bal5').val() < 100 || jQuery('#bal4').val() < 100 || jQuery('#bal3').val() < 100
			|| jQuery('#bal2').val() < 100 || jQuery('#bal').val() < 100){
